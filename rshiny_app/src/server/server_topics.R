...
observeEvent(input$nmf,{
  
  subset_label = dt_topics$label_df_modele_choix
  CLUSTER = subset_label[which(subset_label$label %in% input$Choix_themes_nmf & subset_label$modele==input$Choix_modele),]$cluster
  
  df_document_vector_before_nmf = subset(datas$df_document_vector_before, modele == input$Choix_modele & cluster %in% CLUSTER)
  df_document_vector_nmf = subset(datas$df_document_vector, modele == input$Choix_modele & cluster %in% CLUSTER)
  top_topic_terms_ta = nmf_for_one_topic(df_document_vector_before_nmf, df_document_vector_nmf, input$k_topics)
  dt_topics$top_termes_nmf = top_topic_terms_ta[[1]]
  dt_topics$doc_topic = top_topic_terms_ta[[2]]
  
  dt_topics$yolo = dt_topics$yolo + input$k_topics
  
  data_terms = top_topic_terms_ta[[1]]
  data_docs = top_topic_terms_ta[[2]]
  
  list_df_top_termes = dataframe_topics_nmf(10,data_terms,data_docs)
  dt_topics$length_nmf = list_df_top_termes[[1]]
  
  if (top_topic_terms_ta[[3]] == TRUE){
            output$info_nmf <- renderText({})
            output$table_topics_nmf <- DT::renderDataTable(
              {
                data.frame(
                  
                  # vecteur de chechbox pour la classe checkbox / marche aussi avec shinyInput fonction / on fait appel au checkbox avec input[[paste0(nmf,i)]]
                  Select = paste0('<div><div class="checkbox">
                               <label><input id="nmf',rep(dt_topics$yolo:(dt_topics$yolo+list_df_top_termes[[1]]-1)),'" type="checkbox"/><span></span></label></div></div>'),
                  Classement = list_df_top_termes[[2]],
                  Top_termes = list_df_top_termes[[3]],
                  stringsAsFactors = FALSE,
                  row.names = 1:list_df_top_termes[[1]]
                )
              }, server = FALSE, escape = FALSE, selection = 'none',
              options = list(lengthChange = FALSE, pageLength = nombre_topics_nmf(top_topic_terms_ta[[1]]), paging = FALSE, info = FALSE,
                             ordering = FALSE,columnDefs = list(list(className = 'dt-center', targets = 1)),
                             
                             ### options indispensable pour faire appel à input[[paste0(nmf,i)]] :
              ...
              output$table_topics_nmf <- DT::renderDataTable({}, server = FALSE, escape = FALSE, selection = 'none')
              output$info_nmf <- renderText({
                "Le nombre de documents pour ce topic est insuffisant pour une extraction de topics"
              })
  }
  
}, priority = 0,ignoreNULL = FALSE,ignoreInit = TRUE)


dataframe_topics_nmf = function(nb_top_termes,data_terms,data_docs){
  "Affiche les tops termes de chaque topic et classe les topics selon choix_classement"

  classement_cluster = as.data.frame(table(as.vector(data_docs)))
  classement_cluster = subset(classement_cluster, !(Var1 == -1)) #remove -1 cluster
  classement_cluster = classement_cluster[order(-classement_cluster$Freq),]
  names(classement_cluster) = c('cluster','score')
  name_score = "Nombre de documents "
  ...
  idx_topics = unique(data_terms$cluster)
  ...
  return(list(nombre_labels,classement,top_termes))
  
}

nombre_topics_nmf = function(top_topic_terms_t){
  return(length(unique(top_topic_terms_t$cluster)))
}

observeEvent(input$change_topic,{
  
  withProgress(message = 'Changement', value = 0, {
  incProgress(1/6)
  keep_group = c()
  for (i in dt_topics$yolo:(dt_topics$yolo+dt_topics$length_nmf-1)) {
    if (input[[paste0('nmf',as.character(i))]]) {
      keep_group = c(keep_group,i-dt_topics$yolo+1)
    }
  }
  keep_group = as.list(keep_group)
  
  if (length(keep_group) == 0){
    output$info_nmf <- renderText({
      "Veuillez sélectionner au moins un topic"
    })
  }
  else{
    incProgress(1/6)
    ...
    labels = datas$df_label[which(datas$df_label$modele==input$Choix_modele & datas$df_label$choix==1),] #premier topic
    updatePickerInput(session = session, inputId = "Choix_themes", selected = as.vector(labels$label[1]), choices = as.vector(labels))
  }
  incProgress(1/6)
  })
